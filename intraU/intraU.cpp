...
void intraU(my_vector_of_vectors_t& indm, my_vector_of_set_t& inds, my_vector_of_vectors_t& samples){
	...
	for(i = 0; i < N i++){
		for(j = 0; j < num_nbr; j++){
			...
			if(inds[j].find(indm[i][j]) == inds[j].end())
				indm[j].append(indm[i][j]);
		}
	}
	...
	for(i = 0; i < N; i++){
		...
		for(j = 0; j < indm[i].size(); j++)
			weights[i][j] = max(l1norm(samples[i], samples[j]), 0);
	}
